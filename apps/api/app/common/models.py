...
class OrgUnit(Base):
    ...
    parent_id: Mapped[Optional[UUID]] = mapped_column(Uuid(as_uuid=True), ForeignKey("org_units.id", ondelete="SET NULL"))
...
class OrgAssignment(Base):
    ...
    org_unit_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("org_units.id", ondelete="CASCADE"), nullable=False)
...
class OrgAssignmentUnit(Base):
    ...
    assignment_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("org_assignments.id", ondelete="CASCADE"), primary_key=True)
    org_unit_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("org_units.id", ondelete="CASCADE"), primary_key=True)
...
class UserSecret(Base):
    ...
    twofa_delivery: Mapped[str] = mapped_column(TwoFADelivery, nullable=False, default="email")
...
class UserInvitation(Base):
    ...
    scope_type: Mapped[str] = mapped_column(
        ScopeType, nullable=False, default="self"
    )
    ...
    __table_args__ = (
        Index("ix_user_invitations_email_tenant", "email", "tenant_id"),
    )
...
class AuditLog(Base):
    ...
    occurred_at: Mapped[datetime] = mapped_column(TIMESTAMP(timezone=True), default=datetime.now(timezone.utc))
